"""
Build catalog documentation for all tables in the Data Warehouse.

Collect metadata from the warehouse (tblproperties, describe detail, count, schema)
and generate a per-table markdown file together with lineage.md.

Usage:
    python script/build_catalog.py
    python script/build_catalog.py --output-dir /path/to/catalog
    python script/build_catalog.py --skip-count   # Skip counting rows (faster)
"""
import argparse
import os
import sys
...
def build_schema_example_table(columns, example_data):
    if not columns:
        return '_No schema information_'
...
def build_table_markdown(table_info, downstream_map=None):
    ...
    parts.append(f"| Last Updated At | {updated_at} |")
    parts.append(f"| Record Count | {row_count if row_count is not None else 'N/A'} |")
...
def build_lineage_md(all_tables):
    ...
    lines.append("Diagram showing the dependency (lineage) between tables in the warehouse.")
    lines.append("The `-->` arrow means \"used to create\".\n")
...
def main():
    ...
    parser.add_argument("--skip-count", action="store_true", help="Skip counting rows")
