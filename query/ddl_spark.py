"""
Tool to execute DDL (CREATE SCHEMA, CREATE TABLE, ALTER, DROP...) via the Metabase API with the Spark engine.
...
def run_ddl(sql, engine='spark'):
    """Execute a DDL statement and print the result."""
    sql = sql.strip()
    if not sql:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Execute DDL via the Metabase API (Spark engine)"
    )
